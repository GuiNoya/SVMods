...
namespace DailyTasksReport.TaskEngines
{
    class MiscTaskEngine : TaskEngine
    {
        ...
        public override List<ReportReturnItem> GeneralInfo()
        {
            ...
            if (_config.SiloThreshold > 0 && Game1.getFarm().buildings.Any(b => b.buildingType.Value == "Silo") && Game1.getFarm().piecesOfHay < _config.SiloThreshold)
            {
                ...
                {
                    prItem.Add(new ReportReturnItem { Label = I18n.Tasks_Hay(Game1.getFarm().piecesOfHay) });
                }
            }
